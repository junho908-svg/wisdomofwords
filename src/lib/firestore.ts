...
// ── Mock Check ──
function isMockFirebase(): boolean {
...
export async function createOrder(order: Omit<Order, 'createdAt'>): Promise<void> {
  if (isMockFirebase()) {
    ...
    console.log('[Firestore Mock] Create order:', order);
    return;
  }
...
export async function subscribeNewsletter(email: string): Promise<void> {
  if (isMockFirebase()) {
    await new Promise((resolve) => setTimeout(resolve, 800));
    console.log('[Firestore Mock] Subscribe newsletter:', email);
    return;
  }
  const ref = doc(db, 'newsletter_subscribers', email.toLowerCase().trim());
  await setDoc(ref, {
